...
def train_test_split(x, y, test_pct):
    data = zip(x,y)
    test_data,train_data = split_data(data,test_pct)
    if len(train_data)>0:
...
def split_data(data, prob):
    ...
    for row in data:
        r = random.random()
        result[0 if r < prob else 1].append(row)
    return result
...
def logistic_log_gradient(x, y, beta):
    return reduce(add_vector,[logistic_log_gradient_i(x_i,y_i,beta) for x_i,y_i
            in zip(x,y)])
...
#target_fn(theta)
#gradient_fn(theta)
def minimum_batch(target_fn, gradient_fn, theta_0, tolerance=0.00001):
    theta = theta_0
    value = target_fn(theta_0)
    step_size = [1,0.1,0.01,0.001,0.0001,0.00001]
    while True:
        next_theta_list = [step(theta,gradient_fn(theta),step_size_i) for
                step_size_i in step_size] 
...
def maximum_batch(target_fn,gradient_fn,theta_0,tolerance=0.000001):
    return minimum_batch(negate(target_fn), negate_all(gradient_fn),
            theta_0,tolerance)
...
#gradient_fn(x_i,y_i,theta
def minimum_stochastic(target_fn,gradient_fn,x,y,theta_0,step_size0=0.1):
    no_advance_iteration = 10
    num_of_no_advance_iteration = 0
    data=zip(x,y)
    step_size = step_size0

    theta = theta_0
    min_theta,min_score =None,float("inf")
    while num_of_no_advance_iteration < no_advance_iteration:
        score = sum([target_fn(x_i,y_i,theta) for x_i,y_i in data])
        if score < min_score:
           min_theta = theta
           num_of_no_advance_iteration = 0
           min_score = score
           step_size = step_size0
            ...
            num_of_no_advance_iteration+=1
            step_size*=0.9

        for x_i,y_i in random_order(data):
            gradient_i = gradient_fn(x_i,y_i,theta) 
            theta = sub_vector(theta, scalar_multiply(step_size,gradient_i))
    return theta

def maximum_stochastic(target_fn,gradient_fn,x,y,theta_0,step_size=0.1):
...
if __name__ == "__main__":
    ...
    for x_i, y_i in zip(x_test, y_test):
        predict = logistic(dot(theta,x_i))
